...
// The pod's pull requests need only enough to say whether one has stalled, so
// this is its own field set rather than CORE_FIELDS plus extras. Built on
// CORE_FIELDS it asked for every check context on fifty pull requests across
// the org and took ~11s — over GitHub's limit, which returns a truncated body
// and a "Unexpected end of JSON input" banner on a board that was otherwise
// fine. Without the check rollup the same search takes ~1s.
//
// The cost of that is the CI state, which the pod lane no longer reads: a red
// build on a teammate's pull request is theirs to see, not a lead's signal that
// work has stopped moving.
const POD_FIELDS = `number title url isDraft headRefName mergeable reviewDecision createdAt updatedAt
  author { login }
