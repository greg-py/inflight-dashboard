...
fragment PrFields on PullRequest {
  ...
  reviewThreads(first: 50) { nodes { isResolved comments(last: 1) { nodes { createdAt author { login __typename } } } } }
...
const basePrOf = (node, now) => {
  ...
  // Unresolved threads whose last word is someone else's — the same set the
  // openThreads count is drawn from, kept here so the newest one can be timed
  // against the last push.
  const humanThreads = (node.reviewThreads?.nodes ?? []).filter((thread) => {
  ...
  return {
    ...
    openThreads: node.reviewDecision === "APPROVED" ? 0 : humanThreads.length,
    newestOpenThreadAt:
      humanThreads
