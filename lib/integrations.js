...
// stale-over-broken fallback shared by every consumer (UI tabs, policy engine).
...
const basePrOf = (node, now) => {
  ...
  return {
    ...
    // Both self-clear when you (or a session) reply, becoming the last author.
    openThreads:
...
// Execute a staged status transition. Matches the target against the issue's
// available transitions by destination (or transition) name, case-insensitively.
export const transitionJiraIssue = async (key, targetStatus) => {
  const headers = { Authorization: jiraAuth(), Accept: "application/json" };
  const url = `${CONFIG.jiraBaseUrl}/rest/api/3/issue/${key}/transitions`;
  ...
  if (!match) {
    throw new Error(
      `no transition to "${targetStatus}" from here (available: ${transitions
  ...
  if (!post.ok) throw new Error(`Jira ${post.status}: ${(await post.text()).slice(0, 160)}`);
};
