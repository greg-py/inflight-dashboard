...
export const sectionFor = (item) => {
  // Work I lead that nobody owns is not my queue — it is a staffing question,
  // and it stays one whatever its status says.
  if (item.leadUnassigned) return "lead";
  ...
  if (item.prs.length > 0) {
    ...
    // waiting on anyone — a draft is started work, so it sits with the rest of
    // what is in progress however its checks happen to look.
    if (item.prs.every((pr) => pr.isDraft)) return "in_progress";
  ...
  if (CONFIG.waitingStatuses.includes(item.status?.toLowerCase() ?? "")) return "waiting";
  // Started and not-started are different questions — one asks what to carry
  // on with, the other what to pick up next — so they get their own sections
  // rather than one queue that mixes them. Jira's status *category* decides it
  // rather than the status name, which each board is free to rename.
  return item.statusCategory === "new" ? "to_do" : "in_progress";
};
...
export const buildItems = (jiraIssues, prs, mergedPrs = [], leadIssues = []) => {
  // Tagged as they are merged in, so sectionFor can tell a ticket I own from
  // one I merely lead. The two queries cannot overlap — one is assignee = me,
  // the other assignee is empty — so no row can arrive twice.
  const tagged = [
    ...jiraIssues.map((issue) => ({ issue, leadUnassigned: false })),
    ...leadIssues.map((issue) => ({ issue, leadUnassigned: true })),
  ];
  const items = tagged.map(({ issue, leadUnassigned }) => ({
    id: issue.key,
    key: issue.key,
  ...
  for (const pr of prs) {
    ...
    if (matched.length > 0) {
      ...
      items.push({
        ...
        // A pull request with no ticket is still mine — nobody leads it.
