...
export const awaitingReason = (pr) => {
  ...
  if (!longest || longest.waitingDays === null) return `awaiting review · ${pr.ageDays}d`;
  const others = rest.length > 0 ? ` +${rest.length}` : "";
  return `awaiting @${longest.login}${others} · ${longest.waitingDays}d`;
};
...
export const categorizePr = (pr) => {
  ...
  if (bucket === "waiting" && pr.reviewDecision === "APPROVED" && pr.ci !== "pending") {
    ...
    else reasons.push(awaitingReason(pr));
  }
