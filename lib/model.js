...
export const launchForPr = (pr) => {
  ...
  if (
    ...
    // The fingerprint encodes the full feedback state — a new review, a new
    // commit, or new/answered threads (human or bot) each re-arm the action.
    return {
      kind: "address-review",
      label: "address review",
      prompt: autonomous(`/address-review #${pr.number}`),
      repo: pr.repo,
      fingerprint: `review:${pr.repo}#${pr.number}:${pr.changesRequestedAt ?? "none"}:${pr.lastCommitAt}:${pr.openThreads ?? 0}h${pr.botThreads ?? 0}b`,
    };
  }
...
export const launchForReview = (pr) => {
  if (!Number.isInteger(pr.number)) return null;
  // Keyed on the head commit: a re-request after new pushes re-arms the
  // action (as verify-review), while the same commit never double-reviews.
  const fingerprint = `prereview:${pr.repo}#${pr.number}:${pr.lastCommitAt}`;
...
export const diagnosisKeyFor = (pr) => {
  ...
  // Any actionable review feedback gets a digest: a changes-requested review,
  // or unresolved human/bot threads. Keyed on the full feedback state.
  if (
    (pr.reviewDecision === "CHANGES_REQUESTED" && !changesAddressed(pr)) ||
    (pr.openThreads > 0 && !changesAddressed(pr)) ||
    pr.botThreads > 0
  ) {
    return `review:${pr.repo}#${pr.number}:${pr.changesRequestedAt ?? "none"}:${pr.lastCommitAt}:${pr.openThreads ?? 0}h${pr.botThreads ?? 0}b`;
  }
  return null;
};

// When PR reality is ahead of the ticket's status, name the transition the
// ticket is owed. Pure detection — executing it is always a human click.
export const statusDriftFor = (item) => {
  if (!item.key) return null;
  const rules = CONFIG.driftRules;
  const current = statusRank(item.status);
  const openPrs = (item.prs ?? []).filter((pr) => !pr.isDraft);
  if ((item.mergedPrs?.length ?? 0) > 0 && (item.prs ?? []).length === 0) {
    return {
      target: rules.merged,
      why: `PR ${item.mergedPrs.map((pr) => `#${pr.number}`).join(", ")} merged`,
    };
  }
  if (openPrs.length === 0) return null;
  if (
    openPrs.some(
      (pr) => pr.qaGate === "passed" && pr.reviewDecision === "APPROVED" && pr.ci === "success",
    ) &&
    current < statusRank(rules.qaPassed)
  ) {
    return { target: rules.qaPassed, why: "QA gate passed" };
  }
  if (
    openPrs.some((pr) => pr.reviewDecision === "APPROVED" && pr.ci === "success") &&
    current < statusRank(rules.approvedGreen)
  ) {
    return { target: rules.approvedGreen, why: "PR approved · CI green" };
  }
  if (current < statusRank(rules.prOpen)) {
    return { target: rules.prOpen, why: `PR ${openPrs.map((pr) => `#${pr.number}`).join(", ")} open` };
  }
  return null;
};
