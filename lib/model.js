...
// One flag per pod ticket, worst first. A pod lead does not need the board
// mirrored — they need the handful of rows they would otherwise ask about at
// standup, which is what each of these thresholds encodes.
const podTicketFlag = (item, days) => {
  const status = String(item.status ?? "").toLowerCase();
  if (isHeld(item)) return "held";
  if (status === "blocked") return days === null ? "blocked" : `blocked · ${days}d`;
  if (CONFIG.qaHoldStatuses.includes(status) && days !== null && days >= CONFIG.podQaAgeDays) {
    return `${item.status} · ${days}d`;
  }
  if (CONFIG.preQaStatuses.includes(status) && days !== null && days >= CONFIG.podIdleDays) {
    return `${item.status} · idle ${days}d`;
  }
  return null;
};

const podPrFlags = (pr, now) => {
  const flags = [];
  const idle = daysSince(pr.updatedAt, now) ?? 0;
  if (pr.isDraft) return flags;
  // No CI state here on purpose: reading it means asking GitHub for every check
  // context on fifty pull requests, which is what pushed this search past its
  // time limit. A red build on a teammate's pull request is theirs to see —
  // this lane is for work that has stopped moving.
  if (pr.mergeable === "CONFLICTING") flags.push("conflicts with base");
  if ((pr.pendingReviewers?.length ?? 0) === 0 && pr.reviewDecision !== "APPROVED") {
    flags.push(`no reviewer requested · ${pr.ageDays}d`);
  } else if (pr.reviewDecision === "CHANGES_REQUESTED" && idle >= CONFIG.podReviewIdleDays) {
    flags.push(`changes requested · idle ${idle}d`);
  } else if (
    pr.reviewDecision !== "APPROVED" &&
    (pr.ageDays ?? 0) >= CONFIG.podReviewIdleDays
  ) {
    flags.push(`unreviewed · ${pr.ageDays}d`);
  }
  return flags;
};

// The pod lane: teammates' work that has stopped moving, plus every ticket the
// board's owner is Engineering Lead on but not assigned — those are theirs to
// answer for and are invisible to an assignee-scoped board.
export const buildPodWatch = (podIssues, leadIssues, podPrs, now = Date.now()) => {
  const leadKeys = new Set((leadIssues ?? []).map((issue) => issue.key));
  const byKey = new Map();
  for (const issue of [...(podIssues ?? []), ...(leadIssues ?? [])]) {
    if (!byKey.has(issue.key)) byKey.set(issue.key, issue);
  }
  const prsByKey = new Map();
  for (const pr of podPrs ?? []) {
    for (const key of extractTicketKeys(pr)) {
  ...
  for (const issue of byKey.values()) {
    const days = daysSince(issue.statusSince, now);
    const prs = prsByKey.get(issue.key) ?? [];
    const prRows = prs
      .map((pr) => ({ number: pr.number, url: pr.url, repo: pr.repo, flags: podPrFlags(pr, now) }))
      .filter((pr) => pr.flags.length > 0);
    const flag = podTicketFlag(issue, days);
    const isLead = leadKeys.has(issue.key);
    // A ticket earns a row by being stuck, by carrying a stuck pull request, or
    // by being one this board's owner leads. Everything else is the pod working.
    if (!flag && prRows.length === 0 && !isLead) continue;
    rows.push({
      id: `pod-${issue.key}`,
      key: issue.key,
      url: issue.url,
      ...
      urgent: isUrgent(issue),
      isLead,
      statusDays: days,
      flags: [flag, ...(isLead ? ["you lead"] : [])].filter(Boolean),
      prs: prRows,
    });
  }
  // Worst first: urgent, then whatever has been still the longest.
  rows.sort(
    (a, b) =>
      Number(b.urgent) - Number(a.urgent) ||
      (b.statusDays ?? 0) - (a.statusDays ?? 0) ||
      String(a.key).localeCompare(String(b.key)),
  );
  return rows;
};

// The weekly production-bug goalie, announced by a bot and recorded nowhere
// else. The announcement carries both the id and the display name, so no second
// lookup is needed, and its timestamp is when the rotation started — which is
// what makes the bug count since then meaningful.
export const buildGoalie = (messages, viewerId = null) => {
  const sorted = [...(messages ?? [])].sort((a, b) => Number(b.ts ?? 0) - Number(a.ts ?? 0));
  const announcement = sorted.find((message) =>
    CONFIG.goalieChangePattern.test(message.text ?? ""),
  );
  if (!announcement) return null;
  const [, id, name] = announcement.text.match(CONFIG.goalieChangePattern);
  const since = Number(announcement.ts);
  // Bugs land on whoever is goalie, so counting the announcements since the
  // rotation started is the same as counting theirs.
  const bugs = sorted.filter(
    (message) =>
      Number(message.ts ?? 0) >= since && CONFIG.goalieBugPattern.test(message.text ?? ""),
  );
  return {
    id,
    name,
    isViewer: viewerId ? id === viewerId : null,
