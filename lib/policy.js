...
import { tierFor, routeFor, statusDriftFor } from "./model.js";
import {
  state,
  saveState,
  appendJournal,
...
export const decide = (snapshot, ctx) => {
  ...
  // Sessions push to real branches: a riding subtask must never launch while
  // its parent ticket — or a sibling subtask — has an active session that may
  // be committing to the same parent branch.
  const byId = new Map(snapshot.items.map((item) => [item.id, item]));
  const activeItemIds = new Set(
    ctx.sessions
      .filter((s) => ["queued", "running", "staged", "blocked"].includes(s.state))
      .map((s) => s.itemId),
  );
  const parentBranchBusy = (item) => {
    if (!item.parentKey) return false;
    if (activeItemIds.has(item.parentKey)) return true;
    for (const id of activeItemIds) {
      if (byId.get(id)?.parentKey === item.parentKey) return true;
    }
    return false;
  };
  ...
  // Status drift → staged Jira transitions (suggestions only; a human click
  // executes them). Stage once per (item, current status, target); unstage
  // when the drift resolves or changes.
  for (const item of snapshot.items) {
    if (item.hidden) continue;
    const drift = statusDriftFor(item);
    const key = drift ? `transition:${item.id}:${item.status}→${drift.target}` : null;
    const pending = ctx.pendingTransitions.get(item.id);
    if (drift && !ctx.actedOn.has(key) && pending?.key !== key) {
      actions.push({
    ...
    } else if (!drift && pending) {
      actions.push({ type: "unstage-transition", itemId: item.id });
    }
  }
  for (const [itemId] of ctx.pendingTransitions) {
    if (!snapshot.items.some((item) => item.id === itemId)) {
      actions.push({ type: "unstage-transition", itemId });
    }
  }
  ...
  for (const item of snapshot.items) {
    ...
    if (
      ...
      if (parentBranchBusy(item)) {
        actions.push({ type: "defer", reason: "parent branch busy (sibling session)", itemId: item.id });
      } else {
...
export const runPolicyPass = async (snapshot) => {
  ...
  for (const action of actions) {
    ...
    // Transition staging is a suggestion, not an action — it runs even in
    // observe-only mode (the Jira write only ever happens on a human click).
    if (action.type === "stage-transition") {
      state.pendingTransitions.set(action.itemId, {
      ...
      appendJournal({
        actor: "engine",
        action: "staged transition",
        id: action.itemId,
        detail: `${action.status} → ${action.target} (${action.why})`,
      });
      saveState();
      continue;
    }
    if (action.type === "unstage-transition") {
      state.pendingTransitions.delete(action.itemId);
      saveState();
      continue;
    }
