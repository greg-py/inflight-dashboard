...
const rerunFlakyRuns = (pr, fingerprint) => {
  execFile(
    "gh",
    ...
    (err, stdout) => {
      ...
      state.actedOn.set(fingerprint, Date.now());
      saveState();
    },
  );
};
...
export const runPolicyPass = async (snapshot) => {
  ...
  for (const action of actions) {
    ...
    if (!lastPass.autopilot) continue; // observe-only mode
    ...
    if (action.type === "rerun-flake") {
      acted += 1;
      rerunFlakyRuns(action.pr, action.fingerprint);
      continue;
    }
