...
export const CONFIG = {
  ...
  // Work I am Engineering Lead on that nobody has picked up — mine to answer
  // for, and invisible to an assignee-scoped board. Deliberately not the wider
  // "I lead it, someone else has it": that is their work, not a queue of mine.
  //
  // The window matters more than it looks. Unbounded, this is a junk drawer:
  // ten rows, nine of them tickets I was named on months or years ago and that
  // nothing ever happened to (the oldest reads "(FUTURE)" and was last touched
  // nineteen months ago). Scoped to the open sprint it is empty instead, since
  // these get groomed an sprint or more ahead of being worked. Sixty days —
  // roughly four sprints — keeps the live ones and drops the sediment.
  jiraLeadUnassignedJql:
    'project = PY AND "Engineering Lead" = currentUser() AND assignee IS EMPTY ' +
    "AND statusCategory != Done AND updated >= -%DAYS%d ORDER BY updated DESC",
  leadUnassignedLookbackDays: 60,
