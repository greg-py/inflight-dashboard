...
export const CONFIG = {
  ...
  // The pod this board's owner leads. Everything pod-scoped keys off this.
  podName: "Street Sharks",
  // Work on the pod's board that is not mine. Tickets I am Engineering Lead on
  // are mine to answer for even when someone else is assigned, so they are
  // fetched separately rather than filtered out here.
  jiraPodJql:
    'project = PY AND "Assigned Team" = "%POD%" AND sprint in openSprints() ' +
    "AND statusCategory != Done AND assignee != currentUser() ORDER BY updated ASC",
  jiraLeadJql:
    'project = PY AND "Engineering Lead" = currentUser() AND assignee != currentUser() ' +
    "AND statusCategory != Done ORDER BY updated DESC",
  // Pod pull requests are found by ticket key rather than by a hardcoded roster
  // of GitHub logins, which drifts the moment someone joins the pod.
  githubPodSearch: "is:pr is:open archived:false org:PerformYard -author:@me sort:updated-desc",
  ...
  // Pod watch thresholds, in days. These are the ages at which a pod lead would
  // ask about something at standup rather than let it ride.
  podIdleDays: 3,
  podReviewIdleDays: 2,
  podQaAgeDays: 5,
  // My own QA wait, shown on the row once it is worth chasing.
  qaAgeWarnDays: 3,
