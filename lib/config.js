...
export const CONFIG = {
  ...
  // Status-drift detection: when PR reality is ahead of the ticket's status,
  // the engine stages a one-click Jira transition (never executed on its own).
  // Values are the TARGET status names; transitions are matched against the
  // ticket's available transitions case-insensitively.
  driftRules: {
    prOpen: "In Code Review",
    approvedGreen: "Ready To Test",
    qaPassed: "READY TO MERGE",
    merged: "Done",
  },
