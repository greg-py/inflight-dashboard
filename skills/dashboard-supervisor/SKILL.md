...
2. **Load state** from `~/.cache/inflight-supervisor/state.json`: `{ actedOn: {"<action-key>": epochMs}, lastAwaiting: ["<id>"], launchLog: [{"provider": "claude"|"codex", "at": epochMs}], escalated: {"<item id>": epochMs} }` (create on first run; prune `actedOn` entries and `launchLog` rows older than 14 days / 5 hours respectively).
...
5. **Act**, journaling each action as you go: `POST /api/journal {actor: "supervisor", action: "<short verb phrase>", id: "<item id>", detail: "<signal> → tier <tier> → <agent>/<model or default>/<effort>"}`. Journal skipped-but-noteworthy signals (undiagnosed CI failures, budget deferrals, launch-cap deferrals) with action `deferred`.
6. **Save state**: merge new action keys into `actedOn`, append launches to `launchLog`, update `escalated` and `lastAwaiting`.
